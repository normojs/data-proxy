...
export const apiKeySchema = z.object({
  ...
  quota_hard_limit_enabled: z.boolean().optional().default(false),
  expired_time: z.number(), // -1 for never expires
  created_time: z.number(),
  accessed_time: z.number(),
  group: z.string().nullish().default(''),
  group_available: z.boolean().optional().default(true),
  group_unavailable_reason: z.string().nullish().default(''),
  cross_group_retry: z
    .preprocess((v) => {
      if (v === 1) return true
      if (v === 0) return false
  ...
  default_project_id: z.number().optional().default(0),
})
...
export interface ApiKeyEnterpriseProject {
  id: number
  name: string
  slug: string
  description: string
