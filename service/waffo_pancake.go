...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("data-proxy-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "data-proxy-store"
	defaultWaffoPancakeProductName = "data-proxy-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because Data Proxy has no renewal-
// event handling; Pancake auto-renewing without Data Proxy extending user
