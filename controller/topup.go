...
func GetTopUpInfo(c *gin.Context) {
	...
	data := gin.H{
		...
		"wechat_pay_min_topup":    getWechatPayMinTopup(),
...
func getPayMoney(amount int64, group string) float64 {
	...
	dDiscount := decimal.NewFromFloat(getTopupDiscountForAmount(amount))
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			completed, quotaToAdd, err := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP())
			if err != nil {
			...
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f topup=%q", completed.TradeNo, completed.UserId, c.ClientIP(), quotaToAdd, completed.Money, common.GetJsonString(completed)))
			model.RecordTopupLog(completed.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), completed.Money), c.ClientIP(), completed.PaymentMethod, "epay")
		}
	} else {
