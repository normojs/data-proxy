...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.POST("/playground/provider-check", middleware.UserAuth(), middleware.CriticalRateLimit(), middleware.DisableCache(), controller.CheckPlaygroundProvider)
		apiRouter.GET("/status/test", middleware.AdminAuth(), controller.TestStatus)
		apiRouter.GET("/notice", controller.GetNotice)
		apiRouter.GET("/user-agreement", controller.GetUserAgreement)
		apiRouter.GET("/privacy-policy", controller.GetPrivacyPolicy)
		apiRouter.GET("/about", controller.GetAbout)
		subsitePublicRoute := apiRouter.Group("/subsites/:slug")
		{
			subsitePublicRoute.GET("/public", controller.GetPublicSubsite)
			subsitePublicRoute.POST("/register", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.RegisterSubsiteUser)
			subsitePublicRoute.GET("/member/self", middleware.UserAuth(), controller.GetSubsiteSelfMember)
			subsitePublicRoute.GET("/dashboard", middleware.UserAuth(), controller.GetSubsiteDashboard)
			subsitePublicRoute.POST("/token", middleware.UserAuth(), middleware.CriticalRateLimit(), middleware.DisableCache(), controller.EnsureSubsiteToken)
			subsitePublicRoute.POST("/token/key", middleware.UserAuth(), middleware.CriticalRateLimit(), middleware.DisableCache(), controller.GetSubsiteTokenKey)
			subsitePublicRoute.POST("/token/rotate", middleware.UserAuth(), middleware.CriticalRateLimit(), middleware.DisableCache(), controller.RotateSubsiteToken)
		}
		subsiteManagementRoute := apiRouter.Group("/subsite-management")
		subsiteManagementRoute.Use(middleware.UserAuth())
		{
			subsiteManagementRoute.GET("/subsites", controller.ListManagedSubsites)
			subsiteManagementRoute.POST("/subsites", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.CreateManagedSubsite)
			subsiteManagementRoute.GET("/subsites/:id", controller.GetManagedSubsite)
			subsiteManagementRoute.GET("/subsites/:id/activity", controller.GetManagedSubsiteActivity)
			subsiteManagementRoute.PATCH("/subsites/:id", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.UpdateManagedSubsite)
			subsiteManagementRoute.PUT("/subsites/:id/quota-policy", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.UpsertManagedSubsiteQuotaPolicy)
			subsiteManagementRoute.GET("/subsites/:id/members", controller.ListManagedSubsiteMembers)
			subsiteManagementRoute.PUT("/subsites/:id/members", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.UpsertManagedSubsiteMember)
			subsiteManagementRoute.DELETE("/subsites/:id/members/:user_id", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.DeleteManagedSubsiteMember)
			subsiteManagementRoute.GET("/subsites/:id/channels", controller.ListManagedSubsiteChannels)
			subsiteManagementRoute.POST("/subsites/:id/channels", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.CreateManagedSubsiteChannel)
			subsiteManagementRoute.GET("/subsites/:id/channels/:channel_id/test", middleware.CriticalRateLimit(), controller.TestManagedSubsiteChannel)
			subsiteManagementRoute.GET("/subsites/:id/channels/:channel_id/balance", middleware.CriticalRateLimit(), controller.UpdateManagedSubsiteChannelBalance)
			subsiteManagementRoute.GET("/subsites/:id/channels/:channel_id/upstream-models", middleware.CriticalRateLimit(), controller.GetManagedSubsiteChannelUpstreamModels)
			subsiteManagementRoute.POST("/subsites/:id/channels/:channel_id/models/sync", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.SyncManagedSubsiteChannelModels)
			subsiteManagementRoute.PATCH("/subsites/:id/channels/:channel_id", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.UpdateManagedSubsiteChannel)
			subsiteManagementRoute.DELETE("/subsites/:id/channels/:channel_id", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.DeleteManagedSubsiteChannel)
		}
		...
		channelRoute.Use(middleware.AdminAuth())
		{
			...
			channelRoute.GET("/:id/health", controller.GetChannelRuntimeHealth)
			channelRoute.POST("/:id/health/reset", controller.ClearChannelRuntimeHealth)
		...
		connectedAppDeviceRoute := apiRouter.Group("/connected-apps/:slug")
		{
			...
			connectedAppUserRoute.Use(middleware.UserAuth())
			{
				...
				connectedAppUserRoute.GET("/developer/sdk-config", controller.GetConnectedAppDeveloperSDKConfig)
				connectedAppUserRoute.GET("/developer/openapi", controller.GetConnectedAppDeveloperOpenAPI)
				connectedAppUserRoute.GET("/developer/usage", controller.GetConnectedAppDeveloperUsage)
				connectedAppUserRoute.POST("/developer/keys", middleware.CriticalRateLimit(), middleware.DisableCache(), controller.CreateConnectedAppDeveloperKey)
		...
		snaplessRoute := apiRouter.Group("/snapless")
		{
			...
			snaplessUserRoute.Use(middleware.UserAuth())
			{
				...
				snaplessUserRoute.GET("/devices", controller.ListSnaplessDevices)
				snaplessUserRoute.POST("/devices/:fingerprint/rotate", middleware.CriticalRateLimit(), controller.RotateSnaplessDevice)
				snaplessUserRoute.DELETE("/devices/:fingerprint", middleware.CriticalRateLimit(), controller.RevokeSnaplessDevice)
		...
		enterpriseRoute.Use(middleware.UserAuth())
		{
			...
			departmentManageEnterpriseRoute.Use(middleware.EnterpriseAnyCapabilityAuth(service.EnterpriseCapabilityManage, service.EnterpriseCapabilityDepartmentManage))
			{
				...
				departmentManageEnterpriseRoute.GET("/policy-group-share-requests", controller.ListEnterprisePolicyGroupShareRequests)
				departmentManageEnterpriseRoute.POST("/policy-groups/:id/share-requests", controller.CreateEnterprisePolicyGroupShareRequest)
				departmentManageEnterpriseRoute.POST("/policy-group-share-requests/:id/approve", controller.ApproveEnterprisePolicyGroupShareRequest)
				departmentManageEnterpriseRoute.POST("/policy-group-share-requests/:id/reject", controller.RejectEnterprisePolicyGroupShareRequest)
