...
| 3 | SNAPLESS-003 | P1 | Next | 用户控制台 Connected App 卡片 | 用户能在控制台查看 Snapless grant、设备列表、最近使用时间、token 状态；能轮换或撤销单台设备；状态与 `/api/snapless/health` 语义一致。 |
| 4 | SNAPLESS-004 | P1 | Next | 余额和充值闭环 | health/config 返回余额不足、用户禁用、模型不可用等可操作状态；前端在 Snapless 卡片和授权页显示明确状态，并能跳到充值或设置入口。 |
| 5 | SNAPLESS-005 | P1 | Planned | Connected App 应用管理 MVP | 管理员可新增/停用应用、配置 allowed/default scopes、trusted 状态和授权方式；Snapless 从内置 app 过渡为可管理 app，保留 migration 兼容。 |
...
1. 开发 `SNAPLESS-003`：新增后端设备列表接口，并在用户控制台增加 Snapless Connected App 卡片。
2. 开发 `SNAPLESS-004`：把 health/config 的状态映射到前端可操作提示，补充值入口。
3. 设计 `SNAPLESS-005` 数据模型：保留内置 Snapless seed，同时让管理员可配置更多应用。
