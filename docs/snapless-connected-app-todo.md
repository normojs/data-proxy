...
1. 开发 `SNAPLESS-004`：把 health/config 的状态映射到前端可操作提示，补充值入口。
2. 设计 `SNAPLESS-005` 数据模型：保留内置 Snapless seed，同时让管理员可配置更多应用。
3. 开发 `SNAPLESS-005` 管理端 MVP：先支持新增、停用和 scopes 配置，再迁移 Snapless 到可管理 app。
