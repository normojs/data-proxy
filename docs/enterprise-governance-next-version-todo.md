...
| ID | 优先级 | 任务 | 验收 |
| --- | --- | --- | --- |
| NV-0601 | P1 | 策略 action 扩展 | 支持 reject、dry_run、alert、fallback_model、queue、shared_pool |
| NV-0602 | P1 | 模型自动降级 | 超额或高成本时可降级到指定模型，保留审计和用户提示 |
| NV-0603 | P1 | 低优先级排队 | 非关键请求可排队或延迟执行 |
| NV-0604 | P2 | 企业共享池 | 部门额度耗尽后可借用企业共享额度，并可配置上限 |
...
| ID | 优先级 | 任务 | 验收 |
| --- | --- | --- | --- |
| NV-0701 | P0 | 企业治理 RBAC 模型 | 支持企业管理员、部门管理员、财务查看员、审计员、项目管理员 |
| NV-0702 | P0 | 部门管理员权限边界 | 只能管理本部门及子部门成员、策略和报表 |
| NV-0703 | P1 | 财务视图 | 只读查看成本、额度、分摊报表和导出 |
| NV-0704 | P1 | 审计员视图 | 只读查看审计和策略命中，不能修改配置 |
| NV-0705 | P1 | 权限回归测试 | 覆盖跨部门越权、普通用户访问和管理员边界 |
