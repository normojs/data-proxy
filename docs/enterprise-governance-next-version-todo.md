...
- NV-0403 已完成离职禁用基础交付：同步成员支持 `status: 2`，预览阶段会生成
  ...
  仍保留为后续增强。
- NV-0404 已完成身份冲突处理 MVP：当同步成员的 provider ID、邮箱或用户名匹配到多个不同用户时，
...
| NV-0603 | P1 | 低优先级排队 | 已完成增强交付：queue 命中后进入企业维度同步 admission queue，先写入 `queued`，拿到队列槽后更新为 `admitted` 并继续 relay，请求结束释放后更新为 `released` 并记录运行耗时；admission 会保存 method、path、query、content type、模型、relay mode、channel id、最多 32 KiB 的请求体预览、body_bytes、body_storage 和 SHA256，且不保存 Authorization/API key 等敏感 header；超过 32 KiB 或 `multipart/form-data` 的可重放请求体会写入 `enterprise_governance_queue_payloads` registry，默认存 DB，开启 `ENTERPRISE_QUEUE_PAYLOAD_OBJECT_PROVIDER=local/s3` 后可写入本地目录或 S3-compatible 对象存储，超过 32 MiB 的请求仅保留截断预览；等待超时返回 429 并更新为 `timeout`，排队阶段请求取消或管理员取消更新为 `canceled`；队列审计、响应 header、管理员取消接口、主节点 stale admission 后台恢复和 `enterprise_governance_queue_admissions` 持久化生命周期记录已可在企业治理 API 和审计页按状态、请求 ID、模型、策略、项目和日期范围分页查看；列表响应、retry/cancel 响应、queue retry/cancel/replay/recovery 审计和历史审计读取会对 `request_payload_json` 里的 raw query、JSON/NDJSON/form/text body 敏感字段做可见性脱敏，DB 原始 payload 仍保留用于 replay；timeout/canceled 可由企业管理员手动标记为 `retry_pending`，记录 retry_count、next_retry_at、last_error 并写入 `queue_admission.retry` 审计；主节点后台 worker 会消费到期 retry_pending，置为 `replay_processing` 后校验 inline JSON 或 DB/object 持久化 payload，按 token_id 取回当前 token 并经现有 relay router 异步重放 JSON、NDJSON、multipart 图片编辑和 audio upload 请求，成功标记 released，失败回落 timeout 并写入 `enterprise_governance.queue_admission.replay` 审计；主节点 payload cleanup task 会按 TTL 清理已 released 的旧 DB/object payload 和旧孤儿 payload；后台延迟执行更多动作和更多 replay endpoint 仍保留为后续增强 |
...
| NV-0605 | P2 | 异常检测后自动限流 | 已完成基础交付：基于企业最近窗口和基线窗口检测请求突增、quota 成本突增，以及 consume/error 日志中的异常失败率；命中后进入短时保护，默认返回 429、写入异常响应 header 和 `enterprise_governance.anomaly_throttle` 审计；保护状态已写入 `enterprise_governance_anomaly_protections` 并可在进程重启后恢复；保护 key 和检测窗口已按 project、org_unit、enterprise 优先级收敛，避免单项目/单部门异常扩大为全企业保护；企业设置支持配置启用状态、窗口、冷却时间、请求/成本突增阈值和失败率阈值；dry-run 只记录 would-throttle 观测；异常保护 result、持久化 payload 和审计 payload 会记录当前 request/scope 已命中的 policy actions，并在审计详情显示 action target；当前请求命中 `queue` policy action 时会标记 `orchestrated` 并放行给 queue admission 接管；企业治理 API 和 Audit 页可按 scope 查看保护记录和趋势摘要。其他异常触发动作执行仍保留为后续增强 |
...
当前 V1.7 已交付最小可用 RBAC 闭环：后端企业治理 API 改为 capability 分组鉴权，前端入口和页签按 `/api/user/self` 的 `permissions.enterprise_governance` 控制；审批、财务和审计入口可分别授权；部门管理员按本部门及子部门 scope 管理成员、部门 scoped 策略组、共享策略组、额度策略、审批、用量和审计日志；策略组成员支持 `viewer/editor` 角色；跨部门共享策略组支持有效期控制、共享申请审批和共享 viewer/editor 角色权限；项目管理员按 owner 或项目 admin 成员 scope 管理项目，项目 member 成员仅能查看 scope 内项目、成员、用量和审计；财务视图支持按筛选导出 CSV。V1.6 shared_pool 持久化增强已补齐独立池状态、借用归属、容量不足阻断、结算/退款归还、池状态/借用流水可见性、独立容量配置和趋势摘要；queue 已补齐异常遗留 admission 的后台恢复审计、bounded 请求载荷预览、DB/对象存储持久化大 payload/multipart、timeout/canceled 手动标记 retry_pending 的可见/可审计调度基础、主节点 replay worker 异步重放 inline JSON/DB/object payload、主节点 payload TTL cleanup，以及 queue 列表/响应/审计可见性脱敏；对象存储支持默认关闭、local 和 S3-compatible provider，写入失败会回退 DB，cleanup 会删除外部对象后再删除 registry；异常检测已补齐 scope-aware 保护记录、趋势摘要可见性、策略动作可审计编排，以及命中当前 `queue` 动作时放行给 queue admission 的执行 MVP，保护 key 优先按项目、部门、企业收敛。下一步可继续补其他异常触发动作执行和更多 replay endpoint。
