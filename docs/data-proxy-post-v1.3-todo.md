...
10. 后续：`queue` 的异步执行/后台重试/取消语义仍需独立增强；`shared_pool` 需要管理 UI、池容量单独配置和趋势报表；异常检测需要按项目/部门的细粒度动作编排和趋势报表。
