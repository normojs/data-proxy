...
11. 后续：`queue` 的真正异步 relay 执行、后台重试语义和大 payload 重放仍需独立增强；异常检测已完成保护 key/scope 模型、策略动作可审计编排和 `queue` 动作执行 MVP，其他动作的异常触发执行仍需继续增强。
