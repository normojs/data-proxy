...
11. 后续：`queue` 的真正异步 relay 执行、后台重试语义和大 payload 重放仍需独立增强；异常检测已完成保护 key/scope 模型 MVP，仍需继续做项目/部门的细粒度动作编排。
