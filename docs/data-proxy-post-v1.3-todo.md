...
11. 后续：`queue` 的真正异步 relay 执行、请求载荷持久化和后台重试语义仍需独立增强；异常检测需要按项目/部门的细粒度动作编排和趋势报表。
