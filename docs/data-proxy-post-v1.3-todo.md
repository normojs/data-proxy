...
| 14 | DP-V17-001 | P2 | In Progress (RBAC MVP+) | V1.7 企业治理 RBAC/财务视图 | 已完成企业治理 capability 鉴权、前端入口/页签控制、财务/审计只读角色、部门管理员 scoped 边界、财务分摊 CSV 导出和基础回归测试；项目级细粒度边界和审计 scoped 过滤仍待后续增强。 |
...
8. 后续：项目级细粒度财务边界、审计视图 scoped 过滤和更细的项目/策略组授权模型仍保留为后续增强。
...
- DP-V17-001 已完成 RBAC MVP+：企业治理后端 API 改为 capability 分组鉴权，前端入口和页签按企业角色权限控制；财务查看员、审计员和项目管理员获得最小只读/管理入口；部门管理员按本部门及子部门 scope 管理成员、额度策略、审批和用量；财务 usage breakdown 支持按筛选导出 CSV；新增回归覆盖只读角色、企业管理员、普通用户隔离、部门管理员跨部门越权、部门用量过滤和 CSV 导出。项目级细粒度边界和审计 scoped 过滤仍保留为后续任务。
