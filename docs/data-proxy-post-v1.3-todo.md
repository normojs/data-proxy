...
| 13 | DP-V16-001 | P2 | Done (MVP+) | V1.6 高级策略动作 | 支持 alert、fallback_model、queue、shared_pool 等动作的配置、命中观测、审计事件和响应提示；fallback_model 已能改写模型、重选渠道并按降级模型重新估算预扣费；queue 已有企业维度同步 admission queue、超时 429、响应 header、审计事件、持久化 queued/admitted/released/timeout/canceled/retry_pending/replay_processing 生命周期记录、bounded 请求载荷预览、企业治理页面可见性、管理员取消等待中 queued 请求能力、后台 stale admission 恢复、timeout/canceled 手动标记 retry_pending API/前端按钮/审计和 retry 元数据；主节点后台 worker 已消费到期 retry_pending，校验 inline JSON 或 DB 持久化 payload，支持大 JSON 与 multipart/audio upload 请求体异步重放，按 token_id 取回当前 token 并经现有 relay router 重放，成功标记 released，失败回落 timeout 并写入 queue replay 审计；主节点后台 payload cleanup 会按 TTL 清理已 released 的旧 DB payload 和旧孤儿 payload；shared_pool 已有独立池状态、借用归属、容量不足阻断、结算归还、响应 header、审计、容量独立配置和趋势摘要；异常检测已能基于突增、失败率和成本异常进入可恢复的企业短时保护限流，支持企业级管理员阈值配置，提供企业级/项目/部门保护记录和趋势可见性，并在异常保护 result、payload 和审计中记录当前 scope 命中的策略动作；异常检测触发后的其他真实动作执行和 payload 外部对象存储留给后续增强。 |
...
11. 已完成：`queue` DB payload 支持主节点后台 TTL cleanup；默认保留 7 天、每小时执行、每批最多 500 条，可通过 `ENTERPRISE_QUEUE_PAYLOAD_TTL_SECONDS`、`ENTERPRISE_QUEUE_PAYLOAD_CLEANUP_INTERVAL_SECONDS` 和 `ENTERPRISE_QUEUE_PAYLOAD_CLEANUP_LIMIT` 调整；清理只删除已 `released` 且超过 TTL 的旧 payload，以及无 admission 的旧孤儿 payload，`retry_pending`、`replay_processing`、`timeout/canceled` 等仍保留以支持后续人工重试。
12. 后续：`queue` 已具备 inline JSON、DB 持久化大 payload、multipart/audio upload 的异步 replay worker 和 DB payload TTL cleanup；payload 外部对象存储、敏感字段更细粒度脱敏和更多 replay endpoint 仍可继续增强；异常检测已完成保护 key/scope 模型、策略动作可审计编排和 `queue` 动作执行 MVP，其他动作的异常触发执行仍需继续增强。
...
- DP-V16-001 已完成增强闭环：配额策略支持 `alert`、`fallback_model`、`queue`、`shared_pool` 非阻断动作；策略命中会保留 counter 观测、响应 header 提示和 `enterprise_governance.policy_action` 审计；`fallback_model` 已从推荐升级为 relay 执行动作，会改写请求模型、重选渠道并按降级模型重新估算预扣费；`queue` 已从可见 MVP 升级为企业维度同步 admission queue，命中后先写入 `queued`，拿槽后更新 `admitted`，请求结束后更新 `released` 并记录运行耗时，同时保存 bounded 请求载荷预览用于排查，并对大 JSON 与 multipart 请求写入 DB payload 引用作为异步重放基础；超时或排队阶段取消分别更新 `timeout/canceled`，企业管理员可取消仍在等待的 queued 请求，相关路径记录 `enterprise_governance.queue_admission` 和 `queue_admission.cancel` 审计，企业治理审计页可按状态、请求 ID、模型、策略、项目和日期范围分页查看队列生命周期；主节点后台会周期恢复异常遗留的 stale `queued/admitted/replay_processing` 记录并写入 `enterprise_governance.queue_admission.recover` 审计；timeout/canceled 记录可由企业管理员标记为 `retry_pending`，记录 retry_count、next_retry_at、last_error 并写入 `queue_admission.retry` 审计；主节点 replay worker 会消费到期 `retry_pending`，校验 inline JSON 或 DB 持久化 payload、按 token_id 取回当前 token，并经现有 relay router 异步重放 JSON、NDJSON、multipart 图片编辑和 audio upload 请求，成功标记 released，失败回落 timeout 并写入 `enterprise_governance.queue_admission.replay` 审计；主节点 payload cleanup 会默认保留 7 天并清理已 released 的旧 DB payload 和旧孤儿 payload；`shared_pool` 已从纯审计升级为独立池状态和借用归属，支持容量不足阻断、按实际用量归还、失败退款、剩余池容量 header、reserve/settle/refund 审计、按 `enterprise + policy + metric` 独立配置容量，并在企业治理审计页提供池容量状态、借用流水和趋势摘要；异常检测已能基于请求突增、失败率和成本突增进入企业短时保护限流，记录 `enterprise_governance.anomaly_throttle` 审计，持久化 active protection 以便重启后恢复，支持企业级管理员阈值配置，企业治理 API/Audit 页提供 scoped 保护列表和趋势摘要，异常审计也会带上当前 scope 命中的 policy actions 和 target；当当前请求命中 `queue` 动作时，异常检测会标记 `orchestrated` 并放行给 queue admission 接管。payload 外部对象存储和其他异常触发动作执行仍保留为 V1.6 后续任务。
