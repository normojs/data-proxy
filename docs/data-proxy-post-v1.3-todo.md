...
11. 后续：`queue` 已具备 inline JSON、DB 持久化大 payload 和 multipart/audio upload 的异步 replay worker；payload 外部对象存储、过期清理、敏感字段更细粒度脱敏和更多 replay endpoint 仍可继续增强；异常检测已完成保护 key/scope 模型、策略动作可审计编排和 `queue` 动作执行 MVP，其他动作的异常触发执行仍需继续增强。
