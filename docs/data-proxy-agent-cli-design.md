...
```bash
data-proxy-agent run --config /config/config.yaml
```
...
- 当前 Go agent 对尚未移植的高危 `remote_exec`、`remote_shell_open`、`remote_shell_eval`、`remote_install_package` 等 `tool_call` 明确返回 `TOOL_NOT_SUPPORTED`，避免服务端请求悬空。
