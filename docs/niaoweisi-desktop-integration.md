...
**Token 分组（重要）：** `niaoweisi` Device 登录签发的平台 `sk-` 使用 Connected App 配置的 **`default_token_group`**（内置默认 **`鸟维斯`**，可在管理端「Connected Apps」修改；空则不强制分组）。签发时写入 `tokens.group`。  
`/v1/models` 与计费按 token 分组过滤；空 group 会导致模型列表为空。桌面端不要改 group，也不要让用户选分组。
